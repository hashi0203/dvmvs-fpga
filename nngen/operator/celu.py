...
class celu(bt._ActFuncOperator):
    ...
    def __init__(self, features, rshift_lut_in=0,
        ...
        self.rshift_lut_in = int(rshift_lut_in)
    ...
    def op(self, strm, *args, **kwargs):
        ...
        patterns = [_celu_n(i * addr_scale) for i in range(1, 2 ** self.lut_addrwidth + 1)]
        patterns.reverse()
        ...
        n = strm.Mux(sra < n_th, -th_scale, lut)
        out = strm.Mux(sra >= 0, args[0], n)
    ...
    def get_eval_method(self):
        ...
        method = functools.partial(method,
                                   rshift_lut_in=self.rshift_lut_in,
